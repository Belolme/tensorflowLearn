...
def main():
    with tf.Session() as sess:
        ...
        def getOutputQLabel(batch):
            ...
            next_state_batch = [d[4].reshape([9]) for d in batch]

            return_batch = []
            for i in range(0, len(batch)):
                terminal = batch[i][5]
                
                q_value = sess.run(y, feed_dict={input_tensor: [batch[i][0].reshape([9])]})[0]
                ...
                if terminal != game.TerminalStatus.GOING:
                    ...
                    if side_batch[i] == game.IsTurnTo.BLACK:
                        ...
                        next_q_value = getMinIndex(next_q_1,
                                                    next_state_batch[i].reshape([-1]),
                                                    game.IsTurnTo.BLANK.value)
                    else:
                        print('hello')
                        next_q_1 = sess.run(y, feed_dict={input_tensor: [next_state_batch[i]]})[0]
                        next_q_value = getMaxIndex(next_q_1,
                                                    next_state_batch[i].reshape([-1]),
                                                    game.IsTurnTo.BLANK.value)
                    q_value[batch[i][6]] = reward_batch[i] + 0.99 * next_q_value
                    print(next_q_1)
                    print(next_q_value)
                    return_batch.append(q_value)
        ...
        for _ in range(0, 1000):
            ...
            _, loss_result = sess.run([train_step,loss],  feed_dict={action: [d[2] for d in minibatch],
                                    input_tensor: [d[0].reshape([9]) for d in minibatch],
            ...
            if times % 999 == 0:
                state1 =  minibatch[0][0].reshape([1, 9])
                # print('state1', state1)
                print(sess.run(y, feed_dict={input_tensor:state1}))

                state2 = minibatch[1][0].reshape([1, 9])
